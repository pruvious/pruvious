...
watch(
  () => props.value,
  () => {
    formattedValue0.value = !props.value || isNull(props.value[0]) ? null : dayjs.utc(props.value[0]).format(format)
    formattedValue1.value = !props.value || isNull(props.value[1]) ? null : dayjs.utc(props.value[1]).format(format)
  },
  { immediate: true },
)
</script>
