<template>
  <div class="relative flex w-full flex-col items-start gap-1">
    ...
    <div class="slider flex w-full items-center gap-3" :class="{ 'mb-4': !options.input }">
      <VueSlider
        :contained="true"
        :disabled="disabled"
      ...
      <component
        ...
        @update:modelValue="emit('update:modelValue', $event)"
        :style="{ width: `${inputSize * 0.55 + 2.4}rem` }"
      />
    </div>
...
import { numberFieldComponent } from '#pruvious/dashboard'
// @ts-ignore
...
const props = defineProps<{
   ...
   */
  options: StandardFieldOptions['slider']
