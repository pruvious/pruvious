...
const props = defineProps<{
   ...
   */
  options: StandardFieldOptions['time-range']
...
const id = pruviousUnique('time-range-field')
const labelHovered = ref<boolean>(false)
const name = [
  pruviousUnique(
    (isArray(props.options.name) ? props.options.name[0] : props.options.name) || props.fieldKey || 'time-range-field',
  ),
  pruviousUnique(
    (isArray(props.options.name) ? props.options.name[1] : props.options.name) || props.fieldKey || 'time-range-field',
  ),
]
...
function update() {
  ...
  emit('update:modelValue', [
    isNull(date0) ? null : getTime(date0.toDate()),
    isNull(date1) ? null : getTime(date1.toDate()),
  ])
}
