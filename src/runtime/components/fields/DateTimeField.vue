...
const props = defineProps<{
   ...
   */
  options: StandardFieldOptions['date-time']
...
const id = pruviousUnique('date-time-field')
...
onMounted(() => {
  picker = flatpickr(inputEl.value!, {
    ...
    minDate: dayjs(props.options.min)
      .subtract(props.options.utc ? dayjs(props.options.min).utcOffset() * 60000 : 0)
      .toDate(),
    maxDate: dayjs(props.options.max)
      .subtract(props.options.utc ? dayjs(props.options.max).utcOffset() * 60000 : 0)
      .toDate(),
    onClose: () => {
      blurActiveElement()
      update()
...
function update() {
  if (inputEl.value!.value) {
    ...
    emit('update:modelValue', date.toDate().getTime())
  } else {
