  ...
  <div class="relative flex w-full flex-col items-start gap-1">
    <div v-if="options.label || options.description" class="flex w-full items-end justify-between gap-4">
      ...
      <div class="mb-0.5 flex shrink-0 gap-1.5 text-gray-400">
        <PruviousIconWorld
          v-if="options.utc"
          v-pruvious-tooltip="__('pruvious-dashboard', 'Coordinated Universal Time (UTC)')"
          class="h-4 w-4"
        />

        <PruviousIconCurrentLocation
          v-if="!options.utc"
          v-pruvious-tooltip="
            __('pruvious-dashboard', '$timezone time', { timezone: Intl.DateTimeFormat().resolvedOptions().timeZone })
          "
          class="h-4 w-4"
        />
...
const props = defineProps<{
   ...
   */
  options: StandardFieldOptions['date-time-range']
...
const id = pruviousUnique('date-time-range-field')
const labelHovered = ref<boolean>(false)
const name = [
  pruviousUnique(
    (isArray(props.options.name) ? props.options.name[0] : props.options.name) ||
      props.fieldKey ||
      'date-time-range-field',
  ),
  pruviousUnique(
    (isArray(props.options.name) ? props.options.name[1] : props.options.name) ||
      props.fieldKey ||
      'date-time-range-field',
  ),
]
...
function update() {
  ...
  if (props.options.utc && date0) {
    date0 = date0.add(date0.utcOffset() * 60000)
  }

  if (props.options.utc && date1) {
...
function toMinMaxDate(timestamp: number) {
  return dayjs(timestamp)
    .subtract(props.options.utc ? dayjs(timestamp).utcOffset() * 60000 : 0)
    .toDate()
}
