...
describe('select field', () => {
  it("defaults to '1'", async () => {
    const response = await $fetch('/api/collections/select-fields?select=default,required', {
      method: 'post',
      body: { required: 1 },
  ...
  it("accepts 'foo'", async () => {
    const response = await $fetch('/api/collections/select-fields?select=required', {
      method: 'post',
      body: { required: 'foo' },
      headers: { Authorization: `Bearer ${process.env.TOKEN}` },
    })
  ...
  it('accepts null', async () => {
    const response = await $fetch('/api/collections/select-fields?select=default', {
      method: 'post',
  ...
  it('does not accept null when required', async () => {
    const response = await $fetch('/api/collections/select-fields', {
      method: 'post',
      body: { required: null },
      headers: { Authorization: `Bearer ${process.env.TOKEN}` },
