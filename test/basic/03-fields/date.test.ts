...
describe('date field', () => {
  it('accepts timestamp', async () => {
    const response = await $fetch('/api/collections/date-fields?select=regular,required,min,max', {
      method: 'post',
  ...
  it('normalizes timestamp', async () => {
    const response = await $fetch('/api/collections/date-fields?select=required', {
      method: 'post',
      body: { required: 1678060800001 },
      headers: { Authorization: `Bearer ${process.env.TOKEN}` },
    })

    expect(response).toEqual({ required: 1678060800000 })
  })

  it('does not accept decimal number', async () => {
    const response = await $fetch('/api/collections/date-fields', {
      method: 'post',
