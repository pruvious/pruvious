...
describe('date-time-range field', () => {
  it('accepts timestamp', async () => {
    const response = await $fetch('/api/collections/date-time-range-fields?select=regular,required,min,max', {
      method: 'post',
      body: { required: [1678060800000, '1678060800000'], min: [1686009601000, null] },
    ...
    expect(response).toEqual({
      ...
      min: [1686009601000, null],
      max: [1686009601000, 1686009601000],
    })
  })

  it('does not accept decimal number', async () => {
    const response = await $fetch('/api/collections/date-time-range-fields', {
      method: 'post',
  ...
  it('compares tuple', async () => {
    const response = await $fetch('/api/collections/date-time-range-fields', {
      method: 'post',
      body: { required: [1678060800001, 1678060800000] },
      headers: { Authorization: `Bearer ${process.env.TOKEN}` },
      ignoreResponseError: true,
    })
