...
import { isArray, isDefined, isObject, isString, isUndefined, remap, toArray } from '@pruvious/utils'
...
const sanitized = computed<Record<string, any>[]>(() => {
  ...
  return isArray(v) && v.every(isObject) ? (v as any) : []
})
...
function prepareEmitValue(value: Record<string, any>): string {
...
:deep(.p-field-filter-value > .pui-field > .pui-structure:not(.pui-structure-empty) + .p-repeater-add-item) {
  margin-top: 0.5rem;
}

:deep(.p-field-filter-value > .pui-field > .pui-structure),
:deep(.p-field-filter-value > .pui-field > .pui-structure-empty + .p-repeater-add-item) {
  margin-top: 0;
}
</style>
