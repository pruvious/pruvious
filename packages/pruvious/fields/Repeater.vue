    ...
    >
      <template #header="{ item, index }">
        <span v-if="options.ui.header?.subfieldValue" class="pui-muted pui-truncate">
          {{
            options.ui.header?.showItemNumbers !== false
              ? `${index + 1}. ${item[options.ui.header.subfieldValue]}`
              : item[options.ui.header.subfieldValue]
          }}
        </span>
        <span v-else-if="options.ui.header?.showItemNumbers !== false" class="pui-muted pui-truncate">
          #{{ index + 1 }}
        </span>
  ...
  (_, oldValue) => {
    if (!deepCompare(props.modelValue, prevModelValue.value)) {
      ...
      structuredValue.value = props.modelValue.map((item, i) => ({
        ...item,
        $expanded: structuredValue.value[i]?.$expanded ?? true,
...
function commitModelValue(mValue: Record<string, any>[]) {
  emit(
    'commit',
    mValue.map((item) => omit(item, ['$expanded', '$key'])),
  )
}
