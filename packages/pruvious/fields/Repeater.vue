    ...
    >
          ...
          >
              ...
              @click="
                () => {
                  clipboard.copy({
                    ...
                    data: { $type: itemTypeHash, ...omit(item, ['$expanded', '$key']) },
                  })
            ...
            <PUIDropdownItem
              v-if="
                ...
                itemTypeHash === clipboardData.data.$type &&
              ...
              @click="
                () => {
                  if (clipboardData?.pruviousClipboardDataType === 'structure-item') {
                    const item = { ...omit(clipboardData.data, ['$type']), $expanded: true, $key: nanoid() }
      ...
      <template #itemBefore="{ item, index }">
        <div
          ...
          hidden
          class="p-repeater-next-item-has-error"
        ></div>
      </template>
