<template>
  <component
    ...
    :disabled="
      disabled || options.ui?.disabled || options.autoGenerated || (options.immutable && operation === 'update')
    "
    :error="error"
    :fields="fields"
    :is="component"
...
watch(
  () => [props.dataContainerType, props.dataContainerName, props.path, props.options, props.type],
  () => {
    ...
    if (customComponent) {
      if (isDefined(customComponents[customComponent])) {
        ...
        console.warn(
          `Unable to resolve custom component \`${customComponent}\` for field \`${props.path}\` in \`${props.dataContainerName}\` ${props.dataContainerType}. Available custom components:`,
