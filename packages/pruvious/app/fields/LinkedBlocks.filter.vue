...
const props = defineProps({
   ...
   */
  options: {
    type: Object as PropType<SerializableFieldOptions<'linkedBlocks'>>,
    required: true,
  },
})
...
async function choicesResolver(page: number, keyword: string): Promise<PUIDynamicSelectPaginatedChoices> {
  ...
  if (response.success) {
    return {
      choices: response.data.records.map((record) => ({
        value: record.id,
    ...
    if (query.success) {
      return query.data
        ...
        : {
            ...
            label: __('pruvious-dashboard', 'Record not found') + ` (#${props.modelValue.value})`,
