...
const customOptions: {
   ...
   *
   * - Numeric - Unix timestamp in milliseconds.
   *   - Values must be rounded to the nearest day.
   *     Millisecond timestamps are only used for consistency.
   * - String - Duration string (e.g., '1 day', '30 days').
   *   - Parsed using the [`jose`](https://github.com/panva/jose/blob/main/src/lib/secs.ts) library.
   * - Object - Object with `days` property.
   *
   * By default, start and end dates can be identical (zero time difference).
   *
   * @default 0
   */
  minRange?: number | string | { days?: number }
   ...
   * @default null
   */
  maxRange?: number | string | { days?: number } | null
      ...
      if (isNotNull(value)) {
        for (const v of value) {
          ...
          if (
            ...
            throw new Error(context.__('pruvious-api', 'The values must be rounded to the nearest UTC day'))
          }
        }
      }
    },
