...
function getClientI18nFileContent() {
  ...
  return [
    ...
    ` */`,
    `export const i18n = new I18n().setFallbackLanguages([${pruviousOptions.i18n.fallbackLanguages.map((lang) => `'${lang}'`).join(', ')}]) as typeof _i18n`,
    ``,
    `/**`,
...
function getReExports() {
  ...
  return [
    ...
    `export { useLanguage, useDashboardContentLanguage, extractLanguageCode, preloadTranslatableStrings, preloadTranslatableStringsForPath } from '${resolve('../translations/utils.client')}'`,
