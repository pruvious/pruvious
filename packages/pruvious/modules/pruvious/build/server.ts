...
function getServerFileContent() {
  ...
  return [
    ...
    `import type { CollectionAPI, DynamicCollectionFieldTypes } from './dynamic-collection-types'`,
...
function getServerFileContentPatch() {
  const nuxt = useNuxt()
  const pruviousOptions = nuxt.options.runtimeConfig.pruvious
  const dynamicCollectionTypesPath = `${pruviousOptions.dir.build}/server/dynamic-collection-types.ts`
  ...
  if (process.env.NODE_ENV === 'dev' || process.env.NODE_ENV === 'development') {
    content.push(
      ...
      `  ]`,
      ``,
      `  fs.writeFileSync('${dynamicCollectionTypesPath}', content.join('\\n'))`,
      `}`,
      ``,
      `generateDynamicCollectionFieldTypes()`,
      ``,
      `export type { CollectionAPI, DynamicCollectionFieldTypes } from './dynamic-collection-types'`,
      ``,
      ...
      "    `import type { BlockName } from './blocks'`,",
      "    `import type { DynamicCollectionFieldTypes } from './dynamic-collection-types'`,",
      '    ``,',
