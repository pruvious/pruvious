...
function getServerDynamicRoutesCollectionFileContent() {
  ...
  for (const { code } of pruviousOptions.i18n.languages) {
    ...
    output.push(
      ...
      `            description: ({ __ }) => [`,
      `              __('pruvious-dashboard', 'The URL or full path where users will be redirected.'),`,
...
function getServerFileContent() {
  ...
  return [
    ...
    ` * \`\`\``,
    ` */`,
    `export function uniqueValidator<TCollection extends GenericCollection, TField extends string = TCollection['TColumnNames']>(options?: UniqueValidatorOptions<TField, GenericDatabase>): GenericValidator {`,
    `  return _uniqueValidator(options)`,
    `}`,
    ``,
    getReExports(),
  ].join('\n')
