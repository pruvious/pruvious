...
> {
   ...
   * Note:
   *
   * - You must define ALL custom options here with default values.
   * - Only options defined here will be accessible in the field instance.
   * - You can use `undefined` as the default value for optional options.
   *
   ...
   * - `unique` - Indicates if the field value must be unique across all records.
   *
   ...
   * - `_*` - All option names starting with an underscore (`_`) are reserved for internal use.
   *
   * @example
   * ```ts
   * import { defineField } from '#pruvious/server'
...
> &
  Omit<TOptions, 'populator'> & {
    /**
     * Ensures that the field (or subfield) value is unique within a collection (or subfields array).
     * You can provide a boolean or an object to define the error message, case sensitivity, and uniqueness per language.
     *
     * When enabled, the `uniqueValidator()` is automatically added to the end of the field's validators list.
     * Additionally, if this field is a top-level field in a collection, a unique index is created in the database
     * for this field/column to enforce uniqueness at the database level.
     *
     * By default, no uniqueness validation is applied.
     *
     * @default false
     *
     * @example
     * ```ts
     * // Simple uniqueness validator
     * unique: true
     *
     * // Uniqueness validator with custom options
     * unique: {
    ...
    unique?:
      | boolean
      | (Omit<UniqueValidatorOptions<string, GenericDatabase>, 'fields'> & {
          /**
  ...
  return {
    ...
    > {
      ...
      const validators = [
        ...
        ...(filteredOptions.unique
          ? [uniqueValidator(isObject(filteredOptions.unique) ? omit(filteredOptions.unique, ['fields']) : undefined)]
          : []),
      ]
      ...
      const field = new Field({
        ...
        options: cleanMerge(
          ...
          fieldTypeOptions.uiOptions !== false
            ...
            : ({} as any),
          { unique: filteredOptions.unique ?? false },
