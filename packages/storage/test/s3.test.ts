...
describe('s3 storage', () => {
  const storage = new Storage({
    driver:
      (process.env.VITE_MINIO_CONNECTION_STRING as any) ??
      's3://pruvious:pruvious@127.0.0.1:9000/pruvious?region=pruvious',
  })
  ...
  test('put/get/delete file', async () => {
    ...
    const getSuccess = (content: string) => ({
      success: true,
      data: { ...putSuccess.data, file: new Uint8Array(Buffer.from(content)) },
    })
    ...
    expect(await storage.get('baz')).toEqual({
      ...
      data: {
        ...
        name: 'baz',
        ext: '',
        file: new Uint8Array(Buffer.from('baz')),
        size: 3,
        etag: expect.stringMatching(/^"[a-f0-9]+"$/),
      },
    })
  ...
  test('metadata', async () => {
    ...
    expect(await storage.meta('/meta/foo.txt')).toEqual({
      success: true,
      data: { size: 3, etag: expect.stringMatching(/^"[a-f0-9]+"$/) },
    })
  ...
  test('stream', async () => {
    ...
    const stream = streamResult.data.stream as ReadableStream
    const chunks: Buffer[] = []
    ...
    const streamRange = streamRangeResult.data.stream as ReadableStream
  ...
  test('move', async () => {
    ...
    expect(await storage.get('/target/bar.txt')).toEqual({
      ...
      data: {
        ...
        name: 'bar.txt',
        ext: 'txt',
        file: new Uint8Array(Buffer.from('FOO')),
        size: 3,
        etag: expect.stringMatching(/^"[a-f0-9]+"$/),
      },
    })
  ...
  test('complete multipart upload', async () => {
    ...
    expect(part1).toEqual({ success: true, data: { partNumber: 1, etag: expect.stringMatching(/^"[a-f0-9]+"$/) } })
    expect(part2).toEqual({ success: true, data: { partNumber: 2, etag: expect.stringMatching(/^"[a-f0-9]+"$/) } })
