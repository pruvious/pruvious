...
describe('r2 storage', async () => {
  test('put/get/delete file', async () => {
    const { mf, bucket } = await createMiniflare()
    const storage = new Storage({ driver: bucket as any })
    const putSuccess = {
      success: true,
      data: {
        path: '/foo.txt',
    ...
    expect(await storage.put('baz', 'baz')).toEqual({
      success: true,
      data: { path: '/baz', dir: '/', name: 'baz', ext: '', size: 3, etag: expect.stringMatching(/^"[a-f0-9]+"$/) },
    })
  ...
  test('complete multipart upload', async () => {
    ...
    expect(part1).toEqual({ success: true, data: { partNumber: 1, etag: expect.stringMatching(/^[a-z0-9_-]+$/i) } })
    expect(part2).toEqual({ success: true, data: { partNumber: 2, etag: expect.stringMatching(/^[a-z0-9_-]+$/i) } })
