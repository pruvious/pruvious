...
import { resolveBackupLogPath } from '../../../../utils/backupLog'
...
export default defineEventHandler(async (event) => {
  ...
  const logPath = resolveBackupLogPath(backupId)
  ...
  const tick = async () => {
    ...
    try {
      await pump()
      const statusQuery = await selectFrom('Backups').select(['status']).where('id', '=', backupId).first()
