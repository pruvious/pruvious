...
import { resolveRestoreLogPath } from '../../../../utils/restoreLog'
...
export default defineEventHandler(async (event) => {
  ...
  const logPath = resolveRestoreLogPath(restoreId)
  ...
  const tick = async () => {
    ...
    try {
      await pump()
      const statusQuery = await selectFrom('Restores').select(['status']).where('id', '=', restoreId).first()
