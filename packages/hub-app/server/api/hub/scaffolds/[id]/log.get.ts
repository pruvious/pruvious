...
import { resolveScaffoldLogPath } from '../../../../utils/scaffoldLog'
...
export default defineEventHandler(async (event) => {
  ...
  const scaffoldQuery = await selectFrom('Scaffolds').select(['id']).where('id', '=', scaffoldId).first()
  ...
  const logPath = resolveScaffoldLogPath(scaffoldId)
  ...
  const tick = async () => {
    ...
    try {
      await pump()
      const statusQuery = await selectFrom('Scaffolds').select(['status']).where('id', '=', scaffoldId).first()
